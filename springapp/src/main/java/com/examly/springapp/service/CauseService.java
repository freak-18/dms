...
public class CauseService {
    ...
    public Cause createCause(Cause cause, Long ngoId) {
        if (cause.getEndDate().isBefore(cause.getStartDate())) {
            throw new DataIntegrityViolationException("End date before start date");
        }
        NGO ngo = ngoRepo.findById(ngoId).orElseThrow(() -> new EntityNotFoundException("NGO not found"));
        cause.setNgo(ngo);
    ...
    public Cause getCauseById(Long id) {
        return causeRepo.findById(id).orElseThrow(() -> new EntityNotFoundException("Cause not found"));
    }
}
